...
namespace aruco {
    ...
    void drawKeyPoints(cv::Mat image, std::vector<cv::KeyPoint> kpoints, bool text=false, bool norm=false)
    {
        for(auto &k:kpoints)
            if(norm){
                k.pt.x = k.pt.x*image.cols/2+image.cols/2;
                k.pt.y = -k.pt.y*image.cols/2+image.cols/2;
            }
...
//        cv::TermCriteria criteria = cv::TermCriteria( cv::TermCriteria::MAX_ITER, 20, 2 );
        //cv::cornerSubPix(image, points, winSize, zeroZone, criteria);
        cornerSubPix(image, points, cv::Size(4,4), cv::Size(-1, -1),cv::TermCriteria( cv::TermCriteria::MAX_ITER | cv::TermCriteria::EPS, 12, 0.005));
    ...
    void FractalPoseTracker::setParams(const CameraParameters& cam_params, const FractalMarkerSet& msconf, const float markerSize)
    {
        ...
        if (_fractalMarker.mInfoType == aruco::FractalMarkerSet::PIX || _fractalMarker.mInfoType == aruco::FractalMarkerSet::METERS)
            _fractalMarker = _fractalMarker.normalize();
        ...
        for(auto id_innerMarker:_fractalMarker.fractalMarkerCollection)
        {
            ...
            _id_radius[markerId] = ((2*NBits)/((sqrt(innerMarker.nBits())+2)*(sqrt(_fractalMarker.nBits())+2)))*radiusF;
        }
    ...
    bool FractalPoseTracker::fractalRefinement(const cv::Ptr<MarkerDetector> markerDetector, int markerWarpPix)
    {
        ...
        for (auto id_marker : _fractalMarker.fractalMarkerCollection)
        {
            ...
            {
                cv::Point3f pn = pt/(_fractalMarker.getFractalSize()/2);
                cv::Mat_<float> src(3,1,rot.type());
                src(0,0)=pn.x;src(1,0)=pn.y;src(2,0)=pn.z;
            ...
            if(marker3d.size()<4)
            {
                ...
                {
                    cv::Mat_<float> src(3,1,_rvec.type());
                    src(0,0)=pt.x;src(1,0)=pt.y;src(2,0)=pt.z;

                    cv::Mat  cam_image_point = rot * src +_tvec;
                    cam_image_point = cam_image_point/cv::norm(cam_image_point);

                    if(cam_image_point.at<float>(2,0)>0.85)
    ...
    bool FractalPoseTracker::fractalInnerPose(const cv::Ptr<aruco::MarkerDetector> markerDetector, const std::vector<aruco::Marker>& vmarkers, bool refinement)
    {
        ...
        else
        {
            if(!_rvec.empty())
            {
                ...
                markerDetector->detect(markerDetector->getImagePyramid()[0]);
                Timer.add("detect");
...
//#define _fractal_debug_classification
    ...
    bool FractalPoseTracker::ROI(const std::vector<cv::Mat> imagePyramid, cv::Mat &img, std::vector<cv::Point2f> &innerPoints2d, cv::Point2f &offset, float &ratio)
    {
        ...
        {
            biggest_p3d.empty();
            for(auto c:_fractalMarker.fractalMarkerCollection[idx].points)
            {
                cv::Point3f pt = c/(_fractalMarker.getFractalSize()/2);
                cv::Mat_<float> src(3,1,rot.type());
                src(0,0)=pt.x;src(1,0)=pt.y;src(2,0)=pt.z;
                cv::Mat  cam_image_point = rot * src + _tvec ;
                cam_image_point = cam_image_point/cv::norm(cam_image_point);
        ...
        if(!biggest_p3d.empty())
        {
            ...
            for(auto pt:marker_smallest.points)
                smallest_p3d.push_back(pt/(_fractalMarker.getFractalSize()/2));
    ...
    cv::Mat FractalPoseTracker::fractal_solve_ransac(int ninners, std::vector<std::pair<uint, std::vector<uint>>> inner_kpnt, std::vector<cv::KeyPoint> kpnts, u_int32_t maxIter, float _minInliers, float _thresInliers)
    {
        ...
        do
        {
            //Nueva condicion de parada para evitar bucle infinito, producido cuando busca
            // el grupo de inliers iniciales.
            //Por ejemplo, para el grupo de innerkpts: 23{2} 18{3}, 14{3}, 3{7,6,5}, 2{3},
            //se selecciona el grupo inicial de inliers:
            //23{2},18{3},3{6} ya no hay forma de encontrar el cuarto inlier...
            uint32_t iter2 = 0;
    ...
    void FractalPoseTracker::assignClass(const cv::Mat &im, std::vector<cv::KeyPoint>& kpoints, bool norm, int wsize)
    {
        if(im.type()!=CV_8UC1)
        ...
        for(auto &kp:kpoints)
        {
            ...
            //Convert point range from norm (-1,1) to (0,imageSize)
            if(norm){
                x = x*im.cols/2+im.cols/2;
                y = -y*im.cols/2+im.cols/2;
            }
