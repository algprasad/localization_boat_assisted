...
namespace aruco {
    ...
    public:
         ...
         * @param norm. Is it necessary to normalize keypoints? (-1 .. 1)
         * @param wsize. Window size
         */
        void assignClass(const cv::Mat &im, std::vector<cv::KeyPoint>& kpoints, bool norm=false, int wsize=5);
         ...
         */
        cv::Mat fractal_solve_ransac(int ninners, std::vector<std::pair<uint, std::vector<uint>>> inner_kpnt, std::vector<cv::KeyPoint> kpnts, u_int32_t maxIter=500, float _minInliers=0.10f, float _thresInliers=0.7f);
    ...
    private:
        ...
        std::map<int, double> _id_area; //Idmarker_projectedArea(Optimus)
